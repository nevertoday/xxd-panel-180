...
# XXD Panel 180｜旅の記憶を刻むレトロ円形スタンプ
...
主題と環境の手掛かり1〜2点を小さな円形記念スタンプに凝縮。木版・消しゴム版と銅版画風の線、古い象牙色の紙、かすれと版ずれ、原写真由来の退色した2〜3色、ごく少量の金箔。大きな余白と、章の下に英語見出し・小さなキーワード。
...
- `design-only`：全画面を Panel 180 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-180` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-180 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-180-prompt.en.md)／[中国語](references/xxd-panel-180-prompt.zh-CN.md)を参照してください。
