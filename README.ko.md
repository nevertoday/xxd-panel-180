...
# XXD Panel 180｜빈티지 원형 여행 기념 스탬프
...
핵심 피사체와 환경 단서 1–2개를 작은 원형 기념 도장으로 압축합니다. 목판·고무판과 에칭 선, 낡은 상아색 종이, 잉크 번짐·누락·미세한 어긋남, 원본에서 추출한 빛바랜 2–3색과 극소량의 금박을 사용합니다. 넓은 여백과 도장 아래 영어 제목·작은 키워드를 배치합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-180`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-180 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-180-prompt.en.md)와 [중국어](references/xxd-panel-180-prompt.zh-CN.md)를 확인하세요.
...
- `design-only`: 전체 캔버스에 Panel 180의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
